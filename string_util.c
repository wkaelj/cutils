...
cutil_string_strip(char *dest, u32 *max, const char *src, const char stripper)
{
    ...
    {
        if (src[i] == stripper)
            lastInstanceOfSlicer = i;
        srcLen = i; // likely inefficient
    }

    if (max && *max == 0)
        *max = srcLen - lastInstanceOfSlicer + 1;
    // X|X|L|O|O|\0
...
Result cutil_string_truncate(
    char *dest, u32 *max, const char *str, const char slicer, bool inclusive)
{
    ...
    {
        if (str[i] == slicer)
            lastSlicerInstance = i;
    }

    // reset max if there is no instances of slicer
    if (lastSlicerInstance == 0)
        lastSlicerInstance = strLength;

    // reset max if it is 0
    if (*max == 0)
        *max = lastSlicerInstance + inclusive + 1;
