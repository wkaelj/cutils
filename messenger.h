...
/**
 * @brief Functions to output and log debug messeges. Use the debug macros pls
...
// check if debug messeges are enabled, otherwise disable the corrospining
// messege
#ifdef ENABLE_FATAL
#define log_fatal(messege, ...) \
    debug_log(DEBUG_FATAL, __FILE__, __func__, __LINE__, messege, ##__VA_ARGS__)
#else
...
#define log_error(messege, ...) \
    debug_log(DEBUG_ERROR, __FILE__, __func__, __LINE__, messege, ##__VA_ARGS__)
#else
...
#define log_warning(messege, ...) \
    debug_log(                    \
        DEBUG_WARNING, __FILE__, __func__, __LINE__, messege, ##__VA_ARGS__)
#else
...
#define log_info(messege, ...) \
    debug_log(DEBUG_INFO, __FILE__, __func__, __LINE__, messege, ##__VA_ARGS__)
#else
...
#define log_debug(messege, ...) \
    debug_log(DEBUG_DEBUG, __FILE__, __func__, __LINE__, messege, ##__VA_ARGS__)
#else
...
#define log_perror(messege, ...) \
    unix_log_perror(             \
        __FILE__, __func__, int_to_str(__LINE__), messege, ##__VA_ARGS__)
